...
def ApproximateSolution(rng, x, Ntrees, t_cutoff, type_dict):
    # Takes:
...
def GenerateTree(rng, component, t_cutoff, alpha_cutoff, l1_types, l2_types, coef):
    ...
    while(t < t_cutoff):
        ...
        t += rng.expovariate(sum(tree_alpha))
        if(t > t_cutoff):
            break
...
def GenerateLevel2FromLevel1(rng, dim, t, l1_type, l2_types):
    ...
    # Returns:
    # - a list of tuples, the tuples being the generated l2_types
    level = l1_type[0]
...
def test_case_1():
    ...
    print(f"The resulting value at x = (1, 1), t = 0.1 is: {test_1_u(0.1)}")
...
def test_modified_inviscid_burgers_1d():
    rng = Random()
    rng.seed(1)
...
def test_1D_wave_equation():
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('')
    ax.set_title('Plot of u_t = -2u_x , u(0, x) = exp(-x)')
    plt.show()

#def test_2D_wave_equation():

if __name__ == "__main__":
    test_1D_wave_equation()